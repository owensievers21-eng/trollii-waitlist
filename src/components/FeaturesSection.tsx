      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-8 md:px-10 lg:px-14 pt-4 pb-24">
        <div className="mb-16">
          ...
          <h2 className="text-[36px] sm:text-[48px] font-semibold text-fore leading-tight mb-4" style={{ fontFamily: 'var(--font-heading)' }}>
            Everything you need<br /><span className="text-primary">to hit your goals</span>
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {FEATURES.map((f) => (
            ...
            >
              <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-5" style={{ background: f.bg, border: `1px solid ${f.color}22` }}>
                {f.icon}
              </div>
              <p className="text-[11px] font-semibold tracking-widest uppercase mb-2" style={{ color: f.color }}>{f.label}</p>
              <h3 className="text-[20px] font-semibold text-fore mb-3 leading-snug" style={{ fontFamily: 'var(--font-heading)' }}>{f.title}</h3>
        ...
        <div className="mt-16 flex flex-col sm:flex-row items-start sm:items-center gap-4">
          ...
          <a href="#nutrition" className="text-[15px] text-slate-500 hover:text-primary transition-colors duration-200 cursor-pointer underline underline-offset-2">
