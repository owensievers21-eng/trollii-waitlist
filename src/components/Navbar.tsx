    ...
    <>
      <nav style={{ zIndex: 10 }} className="fixed top-0 left-0 right-0 flex items-center justify-between px-5 sm:px-8 py-4 sm:py-5">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <span style={{ fontFamily: 'var(--font-heading)', letterSpacing: '-0.02em' }} className="text-[21px] sm:text-[26px] text-fore select-none">trollii</span>
          <span style={{ letterSpacing: '-0.02em' }} className="text-[22px] sm:text-[27px] text-primary select-none leading-none" aria-hidden="true">✳︎</span>
        </div>
        ...
        {/* Mobile hamburger */}
        <button className="md:hidden flex flex-col justify-center items-center gap-[5px] w-10 h-10 cursor-pointer" onClick={() => setOpen((o) => !o)} aria-label={open ? 'Close menu' : 'Open menu'} aria-expanded={open}>
      ...
      <div
        style={{ zIndex: 9 }}
        className={`fixed inset-0 bg-white/95 backdrop-blur-sm flex flex-col justify-center px-8 gap-8 transition-opacity duration-300 md:hidden ${open ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
        aria-hidden={!open}
      >
        {NAV_LINKS.map((link) => (
          <a key={link} href={`#${link.toLowerCase()}`} onClick={() => setOpen(false)} className="text-[32px] font-medium text-fore hover:text-primary transition-colors duration-200 cursor-pointer">{link}</a>
        ))}
        <button onClick={() => { setOpen(false); onWaitlist() }} className="text-[28px] font-medium text-primary underline underline-offset-2 cursor-pointer text-left">
          Get early access →
        </button>
      </div>
    </>
