      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-8 md:px-10 lg:px-14 pb-24">
        ...
        <div className="text-center mb-12">
          ...
          <div className="inline-flex items-center gap-3 bg-white border border-slate-200 rounded-full px-2 py-1.5">
            <button onClick={() => setAnnual(false)} className={`px-4 py-1.5 rounded-full text-[14px] font-medium transition-all duration-200 cursor-pointer ${!annual ? 'bg-primary text-white shadow-sm' : 'text-slate-500 hover:text-fore'}`}>Monthly</button>
            <button onClick={() => setAnnual(true)} className={`px-4 py-1.5 rounded-full text-[14px] font-medium transition-all duration-200 cursor-pointer flex items-center gap-2 ${annual ? 'bg-primary text-white shadow-sm' : 'text-slate-500 hover:text-fore'}`}>
              Annual
              <span className="text-[11px] font-semibold px-1.5 py-0.5 rounded-full" style={{ background: annual ? 'rgba(255,255,255,0.25)' : '#DCFCE7', color: annual ? 'white' : '#059669' }}>Save 20%</span>
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
          {PLANS.map((plan) => (
            <div key={plan.name} className="rounded-2xl border p-7 relative" style={{ background: plan.popular ? plan.color : '#FFFFFF', borderColor: plan.popular ? plan.color : '#E2E8F0', boxShadow: plan.popular ? `0 8px 40px ${plan.color}30` : undefined, transform: plan.popular ? 'scale(1.02)' : undefined }}>
              ...
              <p className="text-[12px] font-bold uppercase tracking-widest mb-3" style={{ color: plan.popular ? 'rgba(255,255,255,0.7)' : plan.color }}>{plan.name}</p>

              <div className="flex items-baseline gap-1 mb-1">
                <span className="text-[42px] font-semibold leading-none" style={{ fontFamily: 'var(--font-heading)', color: plan.popular ? 'white' : '#0F172A' }}>
                  {plan.price[annual ? 'annual' : 'monthly'] === 0 ? 'Free' : `$${plan.price[annual ? 'annual' : 'monthly']}`}
                </span>
                {plan.price.monthly > 0 && <span className="text-[14px]" style={{ color: plan.popular ? 'rgba(255,255,255,0.6)' : '#94A3B8' }}>/mo</span>}
              </div>
              ...
              <p className="text-[14px] mb-7" style={{ color: plan.popular ? 'rgba(255,255,255,0.75)' : '#64748B' }}>{plan.tagline}</p>

              {/* CTA */}
              {plan.ctaStyle === 'outline-purple' ? (
                <span className="w-full flex items-center justify-center rounded-xl py-3 text-[15px] font-semibold mb-7 bg-transparent border-2 border-violet-300 text-violet-400 cursor-not-allowed opacity-60">{plan.cta}</span>
              ) : (
                <button onClick={onWaitlist} className={`w-full flex items-center justify-center rounded-xl py-3 text-[15px] font-semibold mb-7 transition-all duration-200 cursor-pointer ${plan.ctaStyle === 'filled' ? 'bg-white text-primary hover:bg-emerald-50' : 'bg-primary text-white hover:bg-emerald-700'}`}>
                  {plan.cta}
                </button>
              )}
              ...
              <div className="space-y-3">
                {plan.features.map((f) => (
                  <div key={f} className="flex items-start gap-2.5">
                    ...
                    <span className="text-[13px] leading-snug" style={{ color: plan.popular ? 'rgba(255,255,255,0.85)' : '#475569' }}>{f}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <p className="text-center text-[13px] text-slate-400 mt-10">
          Join the waitlist now — early access members get 3 months of Pro free at launch.
        </p>
      </div>
    </section>
  )
