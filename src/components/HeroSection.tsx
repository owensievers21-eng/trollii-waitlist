...
function MacroRow({
  label, value, goal, unit, color, track,
}: { label: string; value: number; goal: number; unit: string; color: string; track: string }) {
...
function NutritionCard({ cardRef, onSignIn }: { cardRef: React.RefObject<HTMLDivElement>; onSignIn: () => void }) {
  ...
  return (
    <div
      ref={cardRef}
      className="hidden lg:block relative z-10 flex-shrink-0"
    ...
    >
      <div
        ref={innerRef}
        className="card-3d grain rounded-2xl border border-emerald-100/80 p-7 w-[360px]"
        style={{
      ...
      >
        ...
        <div className="flex items-start justify-between mb-5">
          <div>
            ...
            <p className="text-[26px] font-semibold text-fore leading-none" style={{ fontFamily: 'var(--font-heading)' }}>
              June 4
            </p>
        ...
        <div className="mb-5">
          <div className="flex items-baseline justify-between mb-1.5">
            ...
            <span className="text-[13px] text-sky-500">
              <span className="font-semibold">1.8</span>
              <span className="text-slate-400 text-[11px]"> / 2.5 L</span>
            </span>
          </div>
        ...
        <button
          ...
          className="
            ...
            rounded-xl border border-emerald-200 py-2.5
        ...
        >
          ...
          Sign in to sync your real data
        </button>

        {/* Footer */}
        <div className="flex items-center gap-1.5 mt-3.5">
          <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />
          <span className="text-[11px] text-slate-400">A.M.I. is syncing your goals</span>
        </div>
      </div>
      {/* closes ref'd inner tilt div */}
    </div>
  )
}
